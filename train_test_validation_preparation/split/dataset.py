...
class Dataset:
    ...
    def split_data_by_ratio(
        ...
        assert sum(train_validation_test_ratio) == 1
        ...
        (
            train_slice_length,
            validation_slice_length,
            test_slice_length,
        ) = self.lcm_integer(train_validation_test_ratio)
        train_validation_test_slice_length = sum(
            (train_slice_length, validation_slice_length, test_slice_length)
        )

        entry_indices = list(range(len(entries)))
        slice_start_indices = entry_indices[
            0::train_validation_test_slice_length
        ]
        train_validation_test_slices = [
            entries[idx : idx + train_validation_test_slice_length]
        ...
        validation_entries = []
        test_entries = []
        for train_validation_test_slice in train_validation_test_slices:
            current_train_slices = train_validation_test_slice[
                0:train_slice_length
            ]
            current_train_validation_slice_length = (
                train_slice_length + validation_slice_length
            )
            current_validation_slices = train_validation_test_slice[
                train_slice_length:current_train_validation_slice_length
            ]
            current_train_validation_test_slice_length = (
                current_train_validation_slice_length + test_slice_length
            )
            current_test_slices = train_validation_test_slice[
                current_train_validation_slice_length:current_train_validation_test_slice_length
            ]

            # The validation and test slices must not be empty!
            # Move the last training image to the set of validation and test
            # images (if necessary).
            if len(current_validation_slices) == 0:
                assert len(current_train_slices) >= 2
                current_validation_slices.append(current_train_slices.pop())
            ...
            validation_entries.extend(current_validation_slices)
            test_entries.extend(current_test_slices)
        assert len(train_entries) + len(validation_entries) + len(
            test_entries
        ) == len(entries)
